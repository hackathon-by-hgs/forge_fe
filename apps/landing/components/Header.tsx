...
export default function Header() {
  ...
  return (
    <>
      <header
        id="main-header"
        className={`fixed top-0 left-0 right-0 z-[1000] transition-all duration-500 h-20 ${isScrollingDown ? 'bg-transparent border-transparent shadow-none' : `${s.nav}`
          }`}
      >
        <div className="section-container flex items-center justify-between h-full">
          ...
          {/* Desktop nav — center */}
          <nav
            className={`hidden lg:flex items-center gap-10 transition-all duration-500 ${isScrollingDown ? 'opacity-0 -translate-y-10 pointer-events-none' : 'opacity-100 translate-y-0'
              }`}
            id="desktop-nav"
          >
            {NAV_LINKS.map((link) => (
          ...
          <button
            ...
            className={`lg:hidden relative w-12 h-12 flex flex-col items-center justify-center gap-1.5 transition-all duration-500 rounded-full ${isScrollingDown
      ...
      <div
        id="mobile-menu"
        className={`fixed inset-0 z-40 bg-white transition-all duration-700 ease-[cubic-bezier(0.85,0,0.15,1)] lg:hidden ${mobileOpen ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
        <nav className="flex flex-col justify-center h-full px-10">
          ...
          <div className="mt-12 flex flex-col gap-6">
            ...
            <a
              ...
              style={{
                transitionDelay: mobileOpen ? `${(NAV_LINKS.length + 1) * 100}ms` : '0ms',
                opacity: mobileOpen ? 1 : 0,
              }}
            >
              BANK PORTAL
